...
from test_framework.util import assert_equal, assert_raises_rpc_error, bytes_to_hex_str, otsoshi_round
...
def make_utxo(node, amount, confirmed=True, scriptPubKey=CScript([1])):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < otsoshi_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, otsoshi_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(OlivingcoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 OVC - so this attempt is rejected.
        dbl_tx = CTransaction()
