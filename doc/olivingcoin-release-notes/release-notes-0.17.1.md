Olivingcoin Core version 0.17.1 is now available from:

  <https://download.olivingcoin.org/olivingcoin-0.17.1/>
...
When olivingcoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `olivingcoin.conf` file, not inside included files or from
  ...
  as olivingcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the olivingcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 OVC/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
prefixing the option with the network, such as:
    ...
    main.uacomment=olivingcoin
    test.uacomment=olivingcoin-testnet
...
be used in V0.17 by starting olivingcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Olivingcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/olivingcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Olivingcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://olivingcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Olivingcoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Olivingcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Olivingcoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in OVC. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When olivingcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If olivingcoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart olivingcoind with
...
- When running olivingcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If olivingcoind
...
- #11962 `0a01843` add seed.olivingcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in OVC (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI olivingcoin:// (krab)
...
- #13528 `b877c39` Move OlivingcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libolivingcoin-util to libolivingcoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for olivingcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling olivingcoin-qt (TheCharlatan)
...
- #14698 `ec71f06` Add olivingcoin-tx.exe into Windows installer (ken2812221)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use OlivingcoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to olivingcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use OlivingcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use OlivingcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use OlivingcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for OlivingcoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_olivingcoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.olivingcoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_olivingcoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in olivingcoind systemd service file (Flowdalic)
- #12652 `55f490a` olivingcoin-cli: Provide a better error message when olivingcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to olivingcoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for olivingcoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` olivingcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix olivingcoin-cli --version (Empact)
...
- #12877 `2b54155` Use olivingcoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update olivingcoin-dot-org links in release-process.md (fanquake)
...
- #13614 `17e9106` Update command line help for -printtoconsole and -debuglogfile (otswo, fanquake)
...
- #13625 `7146672` Add release notes for -printtoconsole and -debuglogfile changes (otswo)
...
- #13742 `d2186b3` Adjust olivingcoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed olivingcoin-cli -help output for help2man (hebasto)
