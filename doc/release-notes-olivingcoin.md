Olivingcoin Core version 0.18.1 is now available from:

  <https://download.olivingcoin.org/olivingcoin-0.18.1/>
...
`/Applications/Olivingcoin-Qt` (on Mac) or `olivingcoind`/`olivingcoin-qt` (on
Linux).
...
Olivingcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Olivingcoin Core on unsupported systems.
...
10.10. Additionally, Olivingcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:5232:5232` (this is an extra `:5232` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@olivingcoin.org) the Olivingcoin Core
...
in OVC per kilobyte, not olitoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/olivingcoin-conf.md)
  about the `olivingcoin.conf` file describes how to use it to configure
  Olivingcoin Core.

- A new document introduces Olivingcoin Core's BIP174 [Partially-Signed
  Olivingcoin Transactions
  (PSBT)](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Olivingcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Olivingcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/olivingcoin-project/olivingcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Olivingcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Olivingcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/olivingcoin-project/olivingcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart olivingcoind with the `-deprecatedrpc=generate`
...
- A new `olivingcoin-wallet` tool is now distributed alongside Olivingcoin
...
This section describes planned changes to Olivingcoin Core that may affect
other Olivingcoin software and services.

- Since version 0.16.0, Olivingcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Olivingcoin Core 0.20 (expected about
  a year after 0.18), Olivingcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Olivingcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Olivingcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Olivingcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Olivingcoin Core
...
- A sub-project of Olivingcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Olivingcoin Core.  See their [project
  page](https://github.com/olivingcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Olivingcoin Core's own implementation, although entropy
  gathered by Olivingcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Olivingcoin Core a
...
- On macOS, Olivingcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `olivingcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libolivingcoin_server.a` (jnewbery)
...
- #13926 [Tools] olivingcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #5232 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
...
- #14162 Also log and print messages or questions like olivingcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `olivingcoin-qt -version` output formatting (hebasto)
...
- #11625 Add OlivingcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libolivingcoin_server` (laanwj)
...
- #15154 Configure: olivingcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/olivingcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Olivingcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libolivingcoin_server` (Mr-Leshiy)
...
- #15947 Install olivingcoin-wallet manpage (domob1812)
- #15983 build with -fstack-reuse=none (MarcoFalke)

### Tests and QA
- #15405 appveyor: Clean cache when build configuration changes (Sjors)
- #13953 Fix deprecation in olivingcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Olivingcoin Core (Christewart)
...
- #12246 Bugfix: Only run olivingcoin-tx tests when olivingcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_olivingcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_olivingcoin` (MarcoFalke)
...
- #14186 olivingcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_olivingcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update olivingcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/olivingcoin-conf.md (hebasto)
...
- #15354 Add missing `olivingcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct olivingcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support olivingcoin folder on OSX (JimmyMow)
...
- [The Olivingcoin Core Developers](https://github.com/olivingcoin/olivingcoin/tree/master/doc/release-notes)
- Adrian Gallagher
- aunyks
- coblee
- cryptonexii
