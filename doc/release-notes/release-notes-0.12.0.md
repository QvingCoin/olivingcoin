Olivingcoin Core version 0.12.0 is now available from:

  <https://olivingcoin.org/bin/olivingcoin-core-0.12.0/>
...
pre-0.12 versions of Olivingcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Olivingcoin Core version 0.11 or
earlier.
...
Olivingcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Olivingcoin Core 0.12 also introduces new default policy limits on the length and
...
Olivingcoin Core 0.12 nodes. Olivingcoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/olivingcoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Olivingcoin Core 0.12 does not yet have support for
...
notes of v0.11.0](https://github.com/olivingcoin/olivingcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
- #6214 `87406aa` [OSX] revert renaming of Olivingcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new olivingcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include olivingcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6851 `616d61b` Optimiotsion: Store transaction list order in memory rather than compute it every need (Luke-Jr)
...
- #6317 `ca5e2a1` minor optimiotsions in peertablemodel (Diapolo)
...
- #6390 `6a73d66` tests: Fix olivingcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of olivingcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override OLIVINGCOIND and OLIVINGCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [olivingcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add OLIVINGCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libolivingcoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "OVC" (Ross Nicoll)
