Olivingcoin Core version 0.11.0 is now available from:

  <https://olivingcoin.org/bin/olivingcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer OVC/kB fee to be
...
For example, add the following to `olivingcoin.conf`:
...
related to the blockchain in the olivingcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Olivingcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Olivingcoin Core will 
...
For an example script see [olivingcoin-submittx](https://github.com/laanwj/olivingcoin-submittx).
...
- #5334 `cf87536` libolivingcoinconsensus: Add pkg-config support
- #5514 `ed11d53` Fix 'make distcheck'
- #5505 `a99ef7d` Build winshutdownmonitor.cpp on Windows only
- #5582 `e8a6639` Osx toovchain update
...
- #6269 `95aca44` gitian: Use the new olivingcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only OlivingcoinAmountField
- #5753 `0f44672` Add olivingcoin logo to about screen
...
- #5809 `46bfbe7` Add olivingcoin-cli man page
...
- #5849 `41259ca` contrib/init/olivingcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Olivingcoin-Qt.app to Olivingcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for olivingcoin-tx)
