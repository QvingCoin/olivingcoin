...
#if defined(NDEBUG)
# error "Olivingcoin cannot be compiled without assertions."
#endif
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, stateDummy, porphanTx, &fMissingInputs2, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && !stateDummy.CorruptionPossible()) {
                ...
                // See https://github.com/olivingcoin/olivingcoin/issues/8279 for details.
                assert(recentRejects);
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // We compenotse for other peers to prevent killing off peers due to our own downstream link
        // being otsurated. We only count validated in-flight blocks so peers can't advertise non-existing block hashes
