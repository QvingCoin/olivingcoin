 ...
 */
static const char* OLIVINGCOIN_PID_FILENAME = "olivingcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", OLIVINGCOIN_PID_FILENAME)));
}
 ...
 * between olivingcoind, and olivingcoin-qt and non-server tools.
*/
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("olivingcoin-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", OLIVINGCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", OLIVINGCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/olivingcoin-project/olivingcoin>";
    const std::string URL_WEBSITE = "<https://olivingcoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("olivingcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Olivingcoin is running in a usable environment with all
...
{
    // Make sure only a single Olivingcoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", OLIVINGCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if olivingcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if olivingcoin is started while in a temporary directory.\n",
     ...
     * be disabled when initialiotsion is finished.
     */
