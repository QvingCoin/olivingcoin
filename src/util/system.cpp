...
const char * const OLIVINGCOIN_CONF_FILENAME = "olivingcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "olivingcoind -foo=bar
...
#else
    const char* pszModule = "olivingcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Olivingcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Olivingcoin
    // Mac: ~/Library/Application Support/Olivingcoin
    // Unix: ~/.olivingcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Olivingcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Olivingcoin";
#else
    // Unix
    return pathRet / ".olivingcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", OLIVINGCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Olivingcoin Core copyright is not removed by accident
    if (copyright_devs.find("Olivingcoin Core") == std::string::npos) {
        std::string strYear = strPrefix;
        strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
        strCopyrightHolders += "\n" + strYear + "The Olivingcoin Core developers";
    }
