...
git clone https://github.com/olivingcoin-core/qa-assets ${DIR_FUZZ_IN}
...
env | grep -E '^(OLIVINGCOIN_CONFIG|CCACHE_|WINEDEBUG|LC_ALL|BOOST_TEST_RANDOM|CONFIG_SHELL|(ASAN|LSAN|TSAN|UBSAN)_OPTIONS)' | tee /tmp/env
if [[ $HOST = *-mingw32 ]]; then
  DOCKER_ADMIN="--cap-add SYS_ADMIN"
elif [[ $OLIVINGCOIN_CONFIG = *--with-sanitizers=*address* ]]; then # If ran with (ASan + LSan), Docker needs access to ptrace (https://github.com/google/sanitizers/issues/764)
...
if [ "$OLIVINGCOIN_SCRYPT" = 1 ]; then 
  travis_retry DOCKER_EXEC pip3 install olivingcoin_scrypt
fi
