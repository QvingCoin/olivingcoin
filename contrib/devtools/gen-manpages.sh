...
OVCVER=($($OLIVINGCOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $OLIVINGCOIND $OLIVINGCOINCLI $OLIVINGCOINTX $WALLET_TOOL $OLIVINGCOINQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${OVCVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${OVCVER[1]}//g" ${MANDIR}/${cmdname}.1
done
