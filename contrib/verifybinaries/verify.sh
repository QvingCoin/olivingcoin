#!/usr/bin/env bash
# Copyright (c) 2016 The Olivingcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   olivingcoincore.org and olivingcoin.org and compares them.
...
WORKINGDIR="/tmp/olivingcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://olivingcoincore.org"
HOST2="https://olivingcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="olivingcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'olivingcoin-',
...
if [ $? -ne 0 ]; then
   echo "olivingcoin.org failed to provide signature file, but olivingcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "olivingcoin.org and olivingcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Olivingcoin Core binary release signing key installed?"
   fi
